...
1. **Коммерческий контент:**
   - Создать `/services.html` и `/ru/services.html` с перечнем услуг, ценами и этапами работы.
...
5. **Аналитика:**
   - Настроить цели в Яндекс.Метрике: отправка формы, клик по email, просмотр портфолио.
   - Следить за поведенческими факторами (время, глубина, отказы).
